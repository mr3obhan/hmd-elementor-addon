 ...
 * License:     MIT
 * Text Domain: hmd-elementor-addon
...
class HMD_ELEMENTOR_ADDON {
	 ...
	 * @status Core
	 */
	public static $Template_Engine = 'hmd-elementor-addon';
	...
	public function includes() {
		...
		include_once dirname( __FILE__ ) . '/inc/class-elementor.php';

		/*
		 * Load List Of classes
		 */
		foreach ( self::$providers as $class ) {
			$class_object = '\HMD_ELEMENTOR_ADDON\\' . $class;
			new $class_object;
		}
	...
	public function init_hooks() {
		 ...
		 */
		register_activation_hook( __FILE__, array( '\HMD_ELEMENTOR_ADDON\config\install', 'run_install' ) );

		/*
		 * Uninstall Plugin Hook
		 */
		register_deactivation_hook( __FILE__, array( '\HMD_ELEMENTOR_ADDON\config\uninstall', 'run_uninstall' ) );

		/*
		 * Load i18n
		 */
		if ( self::$use_i18n === true ) {
			new \HMD_ELEMENTOR_ADDON\config\i18n( 'hmd-elementor-addon' );
		}

		//Check $ENVIRONMENT Mode
		if ( self::$ENVIRONMENT == "development" ) {
			include_once dirname( __FILE__ ) . '/inc/core/debug.php';
			new \HMD_ELEMENTOR_ADDON\core\debug();
		}
	...
	function php_version_notice() {
		...
		$error = __( 'Your installed PHP Version is: ', 'hmd-elementor-addon' ) . PHP_VERSION . '. ';
		$error .= __( 'The <strong>WP Plugin</strong> plugin requires PHP version <strong>', 'hmd-elementor-addon' ) . $this->min_php . __( '</strong> or greater.', 'hmd-elementor-addon' );
		?>
        <div class="error">
 ...
 * @since  1.1.0
 */
function hmd_elementor_addon() {
	return HMD_ELEMENTOR_ADDON::instance();
}

// Global for backwards compatibility.
$GLOBALS['hmd-elementor-addon'] = hmd_elementor_addon();
